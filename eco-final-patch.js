(function(){
  ...
  window.openSubscriptionModal=function(){
    ...
    const err=$('#pay-error');if(err){err.style.color='#f87171';err.textContent='Активация через сайт отключена. Напишите @'+ADMIN+' и отправьте Telegram ID + устройство.';}
  };
  ...
  window.checkAccess=async function(showMsg=true){
    ...
    if(!rec){
      ...
      if(showMsg)showPaywall('Подписка не найдена для ID '+id+'. Проверьте access_keys.json / premium_users.json и обновите сайт.');
      return false;
    }
    if(!activeRec(rec)){
      window.unlocked=false;updateBadge();renderLimit();
      const until=(rec&&typeof rec==='object')?(rec.expires||rec.exp||rec.until||''):'';
      if(showMsg)showPaywall('Запись для ID '+id+' найдена, но подписка не активна'+(until?' или срок истёк: '+until:'')+'.');
      return false;
    }
